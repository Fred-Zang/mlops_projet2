# executer $ pytest => A DEVELOPPER / QUAND LES TESTS UNITAIRES SONT LANCES ...
...
def test_transform_star_to_target():
    ...
    """
    """ 1 étoile => Cible 0"""
    assert transform_star_to_target( pd.DataFrame(data = [1], columns = ['star']))[0] == 0

    """ 2 étoiles => Cible 0"""
    assert transform_star_to_target( pd.DataFrame(data = [2], columns = ['star']))[0] == 0

    """ 3 étoiles => Cible 1"""
    assert transform_star_to_target( pd.DataFrame(data = [3], columns = ['star']))[0] == 1

    """ 4 étoiles => Cible 1"""
    assert transform_star_to_target( pd.DataFrame(data = [4], columns = ['star']))[0] == 1

    """ 5 étoiles => Cible 1"""
    assert transform_star_to_target( pd.DataFrame(data = [5], columns = ['star']))[0] == 1
    
    """ Teste 10 valeurs différentes """
    # DataFrame contenant les valeurs des étoiles
    df_test_star = pd.DataFrame(data = [1, 1, 5, 4, 2, 2, 3, 4, 1, 2], columns = ['star'])
...
### PRETRAITEMENT DU CORPUS ET CREATION DES JEUX D'ENTRAINEMENT ET DE TEST POUR LES MODELES SVM ET ANN ###
def prepare_data():
